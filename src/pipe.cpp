...
rdb::pipe::pipe(bool close_on_exec) {
  if (pipe2(fds_, close_on_exec ? O_CLOEXEC : 0) < 0) {
    rdb::error::send_errno("Pipe creation failed");
  }
}
...
std::vector<std::byte> rdb::pipe::read() {
  char buf[1024];
  int chars_read;
  if ((chars_read = ::read(fds_[read_fd], buf, sizeof(buf))) < 0) {
    rdb::error::send_errno("Could not read from pipe");
  }
  auto bytes = reinterpret_cast<std::byte *>(buf);
  return std::vector<std::byte>(bytes, bytes + chars_read);
}

void rdb::pipe::write(std::byte *from, std::size_t bytes) {
  if (::write(fds_[write_fd], from, bytes) < 0) {
    rdb::error::send_errno("Could not write to pipe");
  }
}
